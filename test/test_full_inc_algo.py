...
class TestFullIncAlgo(unittest.TestCase):
    ...
    def test_run_normal(self):
        ...
        schema = StructType([
            StructField('did', StringType(), True),
            StructField(f'num_{FullIncAlgo.HISTORY_COL_SUFFIX}', ArrayType(LongType()), True),
            StructField(f'desc_{FullIncAlgo.HISTORY_COL_SUFFIX}', ArrayType(StringType()), True)
        ])
        expected_full_day1_df = self.sc.parallelize(full_day1).toDF(schema)
        expected_full_day1_day2_df = self.sc.parallelize(full_day1_day2).toDF(schema)
        expected_full_day1_day2_day3_df = self.sc.parallelize(full_day1_day2_day3).toDF(schema)
        expected_full_day2_day3_df = self.sc.parallelize(full_day2_day3).toDF(schema)
